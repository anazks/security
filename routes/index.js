...
router.post('/voteFinal',(req,res)=>{
  if(req.body.mail==req.session.user.mailid){
    let transporter = nodemailer.createTransport({
      service: 'gmail',
      auth: {
        //althafhussain816@gmail.com
    ...
    let usermail = req.session.user.mailid;
    let mailOptions = {
      from: 'OnlineVoting Team',
      to: usermail,
      subject: 'Verification',
      text: `http://localhost:4000/verification/${usermail}`
    };
    transporter.sendMail(mailOptions, function(error, info){
      if (error) {
        console.log(error);
...
router.get('/loader',(req,res)=>{
  ...
  router.get('/verification/:mail',(req,res)=>{
    console.log("varifictaion")
    let mail = req.params.mail;
    let sessionMail = req.session.user.mailid;
    if(mail == sessionMail){
      res.render('success')
    }else{
      res.render('failed')
